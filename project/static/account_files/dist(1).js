...
var mode=!isEmpty(opts.sku)||!isEmpty(opts.mpn)||!isEmpty(opts.asProduct)?'Product':'LocalBusiness';var d=autoMergeRS(mode);if(d!=false){opts.data=d;}
...
{var url=api+'/json-ld/product/richsnippet';var queryString='store='+opts.store+'&sku='+(opts.sku?encodeURIComponent(opts.sku):'')+'&data='+(!!opts.data?encodeURIComponent(true):encodeURIComponent(false))+'&mpn='+(opts.mpn?encodeURIComponent(opts.mpn):'');if(opts.enable_syndication==true){queryString+='&enable_syndication=true';}else if(opts.enableSyndication==true){queryString+='&enable_syndication=true';}
if(queryString.length>2000){request.open('POST',url,true);request.setRequestHeader("Content-type","application/x-www-form-urlencoded");request.send(queryString);}else{request.open('GET',url+'?'+queryString,true);request.send();}}
request=null;};autoMergeRS=function(mode)
{var existing=findJsonLDSnippets(mode);if(existing==false){existing=findMicroDataSnippets(mode);}
return existing;}
getMicroDataItem=function(i)
{var item={};item.name=i.getAttribute('itemprop');if(i.tagName=='IMG'){item.value=i.getAttribute('src');}else if(i.hasAttribute('content')){item.value=i.getAttribute('content');}else{item.value=i.innerText;}
return item;}
